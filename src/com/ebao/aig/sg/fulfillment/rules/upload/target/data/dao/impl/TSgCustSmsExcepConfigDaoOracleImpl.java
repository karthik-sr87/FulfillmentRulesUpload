...
import com.ebao.aig.sg.fulfillment.rules.upload.target.data.dao.TSgCustSmsExcepConfigDao;
...
public class TSgCustSmsExcepConfigDaoOracleImpl implements TSgCustSmsExcepConfigDao{
  ...
  static {
      table = "T_SG_CUST_SMS_EXCEP_CONFIG";
      columns.put("custSmsExpId", "CUST_SMS_EXP_ID");
      columnTypes.put("custSmsExpId", "java.lang.Long");
  ...
  public TSgCustSmsExcepConfig fillEntity(ResultSet rs) throws SQLException {
      TSgCustSmsExcepConfig entity = new TSgCustSmsExcepConfig();
      entity.setCustSmsExpId(new Long(rs.getLong("CUST_SMS_EXP_ID")));
      if (rs.wasNull()) {
          entity.setCustSmsExpId(null);
      }
...
if ( entity.getCustSmsExpId() == null || entity.getCustSmsExpId().longValue()<=0 ) {
      entity.setCustSmsExpId(OracleSequencePKGenerator.getInstance().getNextPKValue("S_SG_CUST_SMS_EXCEP_CONFIG__CU"));
}
      DBean db = new DBean();
	PreparedStatement ps = null;
	StringBuffer buf = new StringBuffer();
	buf.append("insert into T_SG_CUST_SMS_EXCEP_CONFIG(");
	buf.append("CUST_SMS_EXP_ID");
	buf.append(",LOB_CODE");
	...
	buf.append(")");
	Log.debug(TSgCustSmsExcepConfigDao.class, buf.toString());
	try {
	  db.connect();
	  Connection con = db.getConnection();
	  ps = con.prepareStatement(buf.toString());
      int pos = 1;
	  if(entity.getCustSmsExpId() != null) ps.setLong(pos++, entity.getCustSmsExpId().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "custSmsExpId:" + entity.getCustSmsExpId());
	  if(entity.getLobCode() != null) ps.setLong(pos++, entity.getLobCode().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "lobCode:" + entity.getLobCode());
	  ps.setString(pos++, entity.getProductCode());
	  Log.debug(TSgCustSmsExcepConfigDao.class, "productCode:" + entity.getProductCode());
	  if(entity.getPlanGroupId() != null) ps.setLong(pos++, entity.getPlanGroupId().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "planGroupId:" + entity.getPlanGroupId());
	  if(entity.getPlans() != null) ps.setLong(pos++, entity.getPlans().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "plans:" + entity.getPlans());
	  if(entity.getProducerType() != null) ps.setLong(pos++, entity.getProducerType().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "producerType:" + entity.getProducerType());
	  ps.setString(pos++, entity.getProducerCodeFrom());
	  Log.debug(TSgCustSmsExcepConfigDao.class, "producerCodeFrom:" + entity.getProducerCodeFrom());
	  ps.setString(pos++, entity.getProducerCodeTo());
	  Log.debug(TSgCustSmsExcepConfigDao.class, "producerCodeTo:" + entity.getProducerCodeTo());
	  ps.setString(pos++, entity.getProducersExcluded());
	  Log.debug(TSgCustSmsExcepConfigDao.class, "producersExcluded:" + entity.getProducersExcluded());
	  ps.setTimestamp(pos++, entity.getInsertTime() != null ? new java.sql.Timestamp(entity.getInsertTime().getTime()) : null);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "insertTime:" + entity.getInsertTime());
	  ps.setTimestamp(pos++, entity.getUpdateTime() != null ? new java.sql.Timestamp(entity.getUpdateTime().getTime()) : null);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "updateTime:" + entity.getUpdateTime());
	  if(entity.getInsertedBy() != null) ps.setLong(pos++, entity.getInsertedBy().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "insertedBy:" + entity.getInsertedBy());
	  if(entity.getUpdatedBy() != null) ps.setLong(pos++, entity.getUpdatedBy().longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  Log.debug(TSgCustSmsExcepConfigDao.class, "updatedBy:" + entity.getUpdatedBy());
	  ps.execute();
  ...
  public void update(TSgCustSmsExcepConfig entity) throws GenericException{
	...
	PreparedStatement ps = null;
	StringBuffer buf = new StringBuffer();
	buf.append("update T_SG_CUST_SMS_EXCEP_CONFIG set ");
	buf.append("CUST_SMS_EXP_ID=?");
	buf.append(",LOB_CODE=?");
	buf.append(",PRODUCT_CODE=?");
	buf.append(",PLAN_GROUP_ID=?");
	...
	buf.append("   and CUST_SMS_EXP_ID = ?");
	Log.debug(TSgCustSmsExcepConfigDao.class, buf.toString());
	try {
	  db.connect();
	  Connection con = db.getConnection();
  ...
  public void remove( java.lang.Long custSmsExpId ) throws GenericException{
    if("hibernate".equals(Env.getValue("PERSISTENCE_TYPE"))) {
       TSgCustSmsExcepConfig entity = null;
       try {
          Session session = HibernateUtil.currentSession();
          entity = this.load( custSmsExpId );
	  if(entity==null) {
      	     return;
          }
	...
	PreparedStatement ps = null;
	StringBuffer buf = new StringBuffer();
	buf.append("delete from T_SG_CUST_SMS_EXCEP_CONFIG");
	buf.append(" where 1 = 1");
	buf.append("   and CUST_SMS_EXP_ID = ?");
	Log.debug(TSgCustSmsExcepConfigDao.class, buf.toString());
	try {
	  ...
	  ps = con.prepareStatement(buf.toString());
      int pos = 1;
	  if(custSmsExpId != null) ps.setLong(pos++, custSmsExpId.longValue()); else ps.setNull(pos++, Types.NUMERIC);
	  ps.execute();
	} catch (SQLException e) {
	  throw ExceptionFactory.parse(e);
	} catch (ClassNotFoundException e) {
  ...
  public int getSequenceID() throws SQLException,ClassNotFoundException 
	{
		...
		try{ 
			...
			selectQuery.append("SELECT max(CUST_SMS_EXP_ID) AS SERIAL_ID FROM T_SG_CUST_SMS_EXCEP_CONFIG");
		db = new DBean(false);
		db.connect();
		Connection con = db.getConnection();
  ...
  public TSgCustSmsExcepConfig load( java.lang.Long custSmsExpId ) throws com.ebao.pub.framework.ObjectNotFoundException,GenericException{
    if("hibernate".equals(Env.getValue("PERSISTENCE_TYPE"))) {
       ...
       try {
          Session session = HibernateUtil.currentSession();
          if(custSmsExpId==null) {
	    return null;
	  }
          Serializable sId = custSmsExpId;
	  o = session.load(TSgCustSmsExcepConfig.class,sId);
       } 
       ...
       return (TSgCustSmsExcepConfig)o;
    }
	...
	ResultSet rs = null;
	StringBuffer buf = new StringBuffer();
	buf.append("select * from T_SG_CUST_SMS_EXCEP_CONFIG ");
	buf.append(" where 1=1 ");
	buf.append("   and CUST_SMS_EXP_ID = ?");
	Log.debug(TSgCustSmsExcepConfigDao.class, buf.toString());
	try {
